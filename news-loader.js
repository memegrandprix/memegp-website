...
(function () {
  ...
  // =======================================================
  // HERO COUNTERS RENDER (gp-central.html banner)
  // Updates the live stat pills inside the standardised
  // banner. Targets `.banner-stat strong` elements.
  //   - First pill: total stories count
  //   - Second pill: stories published in the last 7 days
  //   - Third pill: NEXT RACE (static markup, not touched)
  // =======================================================
  function renderHeroCounters(mount, items) {
    var total = items.length;
    var weekAgo = Date.now() - (7 * 24 * 60 * 60 * 1000);
    var thisWeek = items.filter(function (it) {
      return new Date(it.timestamp).getTime() >= weekAgo;
    }).length;

    // The banner uses `<strong>` for the dynamic value inside each pill.
    var strongs = mount.querySelectorAll('.banner-stat strong');
    if (strongs.length >= 1) strongs[0].textContent = total;
    if (strongs.length >= 2) strongs[1].textContent = thisWeek;
    // strongs[2] is NEXT RACE (static, leave untouched)
  }
  ...
  function renderFeed(mount, items) {
    ...
    if (location.hash) {
      // Use getElementById instead of querySelector — IDs starting with digits
      // (like '2026-05-01-...') are valid HTML IDs but invalid CSS selectors,
      // so querySelector('#2026-...') throws a SyntaxError that would crash
      // the rest of the render.
      var hashId = location.hash.slice(1); // strip the '#'
      var anchor = hashId ? document.getElementById(hashId) : null;
  ...
  function renderCard(it) {
    ...
    // Floating bottom-right meme logo. If team.logo is set, render an <img>
    // with an onerror handler that swaps to a ticker-text fallback if the
    // file is missing (e.g. floki.png before the asset is added).
    var logoBlock = '';
    if (it.team) {
      var ticker = escapeHTML(it.team.ticker || '?');
      // Strip leading $ for fallback display so it reads cleaner: "BRETT" not "$BRETT"
      var tickerShort = (it.team.ticker || '?').replace(/^\$/, '');
      if (it.team.logo) {
        logoBlock =
          '<div class="gpf-card-logo" style="--logo-color:' + teamColor + '" aria-hidden="true">' +
          '  <img src="' + escapeHTML(it.team.logo) + '" alt="' + ticker + '"' +
          '       onerror="this.parentNode.classList.add(\'gpf-card-logo--fallback\');' +
          '                this.parentNode.setAttribute(\'data-ticker\',\'' + escapeHTML(tickerShort) + '\');' +
          '                this.remove();">' +
          '</div>';
      } else {
        logoBlock =
          '<div class="gpf-card-logo gpf-card-logo--fallback" style="--logo-color:' + teamColor + '"' +
  ...
  function init() {
    ...
    var heroCounters = document.getElementById('gp-hero-counters');
    if (!ticker && !feed && !heroCounters) return; // no GP CENTRAL on this page
    ...
    fetch('news.json', { cache: 'no-cache' })
      ...
      .then(function (data) {
        ...
        if (heroCounters) renderHeroCounters(heroCounters, items);
      })
      .catch(function (err) {
